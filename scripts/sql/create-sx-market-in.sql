-- Manual/emergency: tạo bảng sx_market_in + seed (mirror migration 20260717030000)
-- Chạy khi cần apply tay trên DB (vd. trước khi migrate deploy).

CREATE TABLE IF NOT EXISTS "sx_market_in" (
    "id" SERIAL NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "sx_market_in_ma_market_key" ON "sx_market_in"("ma_market");
CREATE INDEX IF NOT EXISTS "sx_market_in_id_khach_hang_idx" ON "sx_market_in"("id_khach_hang");
CREATE INDEX IF NOT EXISTS "sx_market_in_trang_thai_idx" ON "sx_market_in"("trang_thai");

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'sx_market_in_id_khach_hang_fkey'
  ) THEN
...
FROM (
  VALUES
    ...
    (4, 'KH0003', 'SP-BAG-015', 'MI0004', 'Market in túi shopping in offset', 'https://drive.google.com/file/d/sample-mi0004', 'da_duyet', '2025-05-15'),
