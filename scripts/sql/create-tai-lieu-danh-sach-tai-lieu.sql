-- Danh sách tài liệu
-- Official path: Prisma Migrate (prisma/migrations/20260717040000_danh_sach_tai_lieu).
-- This file is for manual/emergency apply.
-- Prerequisite: bảng tai_lieu_thiet_lap_loai_tai_lieu (+ seed loại).

CREATE TABLE IF NOT EXISTS "tai_lieu_danh_sach_tai_lieu" (
    "id" SERIAL NOT NULL,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tai_lieu_danh_sach_tai_lieu_id_loai_tai_lieu_fkey'
  ) THEN
    ALTER TABLE "tai_lieu_danh_sach_tai_lieu"
      ADD CONSTRAINT "tai_lieu_danh_sach_tai_lieu_id_loai_tai_lieu_fkey"
      FOREIGN KEY ("id_loai_tai_lieu") REFERENCES "tai_lieu_thiet_lap_loai_tai_lieu"("id")
      ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS "tai_lieu_danh_sach_tai_lieu_id_loai_tai_lieu_idx"
  ON "tai_lieu_danh_sach_tai_lieu"("id_loai_tai_lieu");

CREATE INDEX IF NOT EXISTS "tai_lieu_danh_sach_tai_lieu_trang_thai_idx"
  ON "tai_lieu_danh_sach_tai_lieu"("trang_thai");
