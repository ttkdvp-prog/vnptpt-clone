...
import { useDeleteTaiLieu } from './use-tai-lieu';

export function useTaiLieuPageHandlers(ctx: FeaturePageHandlerContext<TaiLieu>): FeaturePageHandlers<TaiLieu> {
  const {
    rawData,
    rawDataRef,
    viewingItemRef,
  ...
  const deleteMutation = useDeleteTaiLieu();
  const confirm = useConfirmStore((s) => s.confirm);

  const handleEdit = useCallback(
    (item: TaiLieu, origin?: FormViewOrigin) => {
  ...
  const handleView = useCallback((item: TaiLieu) => startTransition(() => setViewingItem(item)), [setViewingItem]);
  ...
  const handleDelete = useCallback(
    (id: string) => {
      ...
      if (!item) return;
      confirm({
        title: txt('congViecTaiLieu.deleteConfirmTitle'),
        message: `${txt('congViecTaiLieu.deleteConfirmMessage')} "${item.ten_ho_so}"?`,
        variant: 'danger',
  ...
  const handleDeleteMany = useCallback(
    (ids: string[]) => {
      confirm({
        title: txt('congViecTaiLieu.bulkDeleteTitle'),
        message: txt('congViecTaiLieu.bulkDeleteMessage', { count: ids.length }),
        variant: 'danger',
