...
import { useLoaiTaiLieu } from '../hooks/use-loai-tai-lieu';
...
const LoaiTaiLieuToolbar: React.FC<Props> = ({
  onAdd,
  onExport,
  onImport,
  onDeleteMany,
  ...
  const { canCreate, canImport, canExport, canDelete } =
    useResourcePermissions('documentSettings');
  ...
  const { views, activeViewId, onApplyView, onSaveView, onDeleteView } = useSavedViewsController({
    storageKey: 'table-loai-tai-lieu',
  ...
  const filterGroups = useMemo(
    () => [
      {
        key: 'nguoi_tao',
        label: txt('documentSettings.filterCreator'),
        icon: UserRound,
    ...
    () => [
      {
        ...
        renderChip: (layout) => (
          <FilterChipMultiSelect
            ...
            placeholder={txt('documentSettings.filterCreator')}
            icon={UserRound}
