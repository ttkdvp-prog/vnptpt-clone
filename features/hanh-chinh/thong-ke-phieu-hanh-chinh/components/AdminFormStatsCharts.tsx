...
export interface AdminFormStatsChartsProps {
  ...
  departmentData: Array<{ id: string | null; name: string; value: number }>;
  onTypeDrillDown?: (typeId: string) => void;
  onStatusDrillDown?: (status: string) => void;
  onDepartmentDrillDown?: (departmentId: string | null) => void;
}

const AdminFormStatsCharts: React.FC<AdminFormStatsChartsProps> = ({
  chartsVisible,
  chartHeight,
  isMdUp,
  primaryHex,
  ...
  if (!hasAny) {
    return (
      <div className="rounded-xl border border-border bg-card px-4 py-10 text-center text-sm text-muted-foreground">
        {txt('adminFormStats.emptyCharts')}
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <div className="rounded-xl border border-border bg-card p-3.5">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-xs font-semibold text-foreground">
            {txt('adminFormStats.statusChart')}
          </h3>
        </div>
        <ResponsiveContainer width="100%" height={chartHeight}>
          <BarChart data={statusData} barSize={isMdUp ? 32 : 28}>
            ...
            <Bar
              dataKey="value"
              radius={[6, 6, 0, 0]}
              name={txt('adminFormStats.quantity')}
              onClick={(data) => {
      ...
      {departmentData.length > 0 && (
        <div className="rounded-xl border border-border bg-card p-3.5">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-foreground">
              {txt('adminFormStats.departmentChart')}
            </h3>
          </div>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <BarChart data={departmentData} layout="vertical" barSize={isMdUp ? 16 : 14}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" opacity={0.5} />
              <XAxis
                type="number"
                tick={{ fontSize: 10, fill: 'var(--muted-foreground)' }}
              ...
              <Bar
                ...
                onClick={(data) => {
                  ...
                  onDepartmentDrillDown?.(id);
                }}
                style={{ cursor: onDepartmentDrillDown ? 'pointer' : 'default' }}
              />
            </BarChart>
          </ResponsiveContainer>
        </div>
...
export default AdminFormStatsCharts;
