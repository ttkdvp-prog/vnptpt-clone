...
const TrangThaiKhachHangDetail: React.FC<Props> = ({
  data,
  onClose,
  onEdit,
  onDelete,
  ...
  return (
    <GenericDrawer
      title={txt('customerSettings.trangThai.detail.title')}
      subtitle={txt('customerSettings.trangThai.detail.subtitle')}
      ...
      <div className="space-y-5">
        <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-bold text-foreground truncate">{data.ten_trang_thai}</h2>
        ...
        <DetailSection title={txt('customerSettings.trangThai.form.generalInfo')} icon={<FileText size={14} />}>
          <DetailFieldGrid>
            <DetailField
              label={txt('customerSettings.trangThai.form.name')}
              value={data.ten_trang_thai}
        ...
        <DetailSystemSection
          title={txt('customerSettings.trangThai.detail.systemInfo')}
          createdAt={data.tg_tao}
          updatedAt={data.tg_cap_nhat}
          createdBy={data.ten_nguoi_tao ?? data.nguoi_tao ?? undefined}
          labels={{
            createdAt: txt('customerSettings.trangThai.detail.createdAt'),
            updated: txt('customerSettings.trangThai.detail.updated'),
          }}
        />
      </div>
    </GenericDrawer>
...
export default TrangThaiKhachHangDetail;
