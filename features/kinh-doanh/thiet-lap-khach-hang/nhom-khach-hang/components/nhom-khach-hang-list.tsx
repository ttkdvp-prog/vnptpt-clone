...
import { NhomKhachHangRowActions } from './nhom-khach-hang-row-actions';
...
interface Props {
  ...
  onView: (item: NhomKhachHang) => void;
  onDuplicate?: (item: NhomKhachHang) => void;
}

const NhomKhachHangList: React.FC<Props> = ({
  data,
  isLoading,
  onEdit,
  onDelete,
  ...
  const renderCell = useCallback(
    (columnId: string, item: NhomKhachHang) => {
      switch (columnId) {
        case 'ten_nhom':
          return (
  ...
  const renderMobileCard = useCallback(
    (item: NhomKhachHang, isSelected: boolean) => (
      <MobileListCard
        selected={isSelected}
        ...
        titleRow={
          <h4 className="truncate text-base font-semibold text-foreground">{item.ten_nhom}</h4>
        }
        subheader={
          item.mo_ta ? (
  ...
  return (
    <GenericTable
      ...
      columns={columns}
      isLoading={isLoading}
      loadingText={txt('customerSettings.nhom.loading')}
      emptyTitle={txt('customerSettings.nhom.empty')}
      emptyDescription={txt('customerSettings.nhom.emptyHint')}
...
export default NhomKhachHangList;
