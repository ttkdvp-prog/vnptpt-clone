...
import { useNhomKhachHang } from '../hooks/use-nhom-khach-hang';
...
const NhomKhachHangToolbar: React.FC<Props> = ({
  onAdd,
  onExport,
  onImport,
  onDeleteMany,
  ...
  const { data: items = [] } = useNhomKhachHang();
  ...
  const { views, activeViewId, onApplyView, onSaveView, onDeleteView } = useSavedViewsController({
    storageKey: 'table-nhom-khach-hang',
...
export default NhomKhachHangToolbar;
