...
import {
  ...
  deleteTrangThaiKhachHang,
  findTrangThaiKhachHangById,
  findTrangThaiKhachHangPage,
  getTrangThaiKhachHangNguoiTao,
  updateTrangThaiKhachHang,
} from '@/server/repositories/trang-thai-khach-hang';

const trangThaiKhachHangRoutes = new Hono<{ Variables: AuthVariables }>();
trangThaiKhachHangRoutes.use('*', requireAuth);

const createBodySchema = z.object({
  ten_trang_thai: z.string().min(1),
  mo_ta: z.union([z.string(), z.null()]).optional(),
});

const updateBodySchema = createBodySchema.partial();

const UNIQUE_MESSAGE = 'Tên trạng thái đã tồn tại';

async function createOne(
  body: z.infer<typeof createBodySchema>,
  employeeId: string,
): Promise<Awaited<ReturnType<typeof createTrangThaiKhachHang>>> {
  return createTrangThaiKhachHang({
    ten_trang_thai: body.ten_trang_thai.trim(),
    mo_ta: body.mo_ta ?? null,
    nguoi_tao: employeeId,
  });
}

trangThaiKhachHangRoutes.get('/', async (c) => {
  const denied = await assertThietLapKhachHangPermission(c, 'xem');
  if (denied) return denied;

  const limit = Number(c.req.query('limit') ?? 50);
  const offset = Number(c.req.query('offset') ?? 0);
  const orderBy = c.req.query('orderBy') ?? 'ten_trang_thai';
  const ascending = c.req.query('ascending') !== 'false';
  const search = c.req.query('search') ?? undefined;

  const result = await findTrangThaiKhachHangPage({
...
trangThaiKhachHangRoutes.get('/:id', async (c) => {
  const id = Number(c.req.param('id'));
  if (!Number.isFinite(id)) return c.json({ error: 'Invalid id' }, 400);

  const item = await findTrangThaiKhachHangById(id);
...
trangThaiKhachHangRoutes.post('/', async (c) => {
...
trangThaiKhachHangRoutes.post('/import', async (c) => {
  const denied = await assertThietLapKhachHangPermission(c, 'them');
  if (denied) return denied;
...
trangThaiKhachHangRoutes.patch('/:id', async (c) => {
  const id = Number(c.req.param('id'));
  if (!Number.isFinite(id)) return c.json({ error: 'Invalid id' }, 400);

  const nguoiTao = await getTrangThaiKhachHangNguoiTao(id);
  ...
  try {
    const item = await updateTrangThaiKhachHang(id, {
      ten_trang_thai: body.ten_trang_thai != null ? body.ten_trang_thai.trim() : undefined,
      mo_ta: body.mo_ta,
    });
    if (!item) return c.json({ error: 'Not found' }, 404);
    return c.json(item);
...
trangThaiKhachHangRoutes.delete('/:id', async (c) => {
  const id = Number(c.req.param('id'));
  if (!Number.isFinite(id)) return c.json({ error: 'Invalid id' }, 400);
  ...
  const ok = await deleteTrangThaiKhachHang(id);
  if (!ok) return c.json({ error: 'Not found' }, 404);
  return c.json({ ok: true });
});

export { trangThaiKhachHangRoutes };
