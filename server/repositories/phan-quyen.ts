...
export type PhanQuyenRow = {
  id: number;
  module_key: string;
  /** Tên chức danh (text tự do, không phải khóa ngoại — bảng chức vụ đã bị xóa). */
  chuc_vu_id: string;
  quyen: string;
  tg_tao: Date;
  tg_cap_nhat: Date;
...
export async function findPhanQuyenByModule(params: {
  ...
  if (params.chucVuIds && params.chucVuIds.length > 0) {
    ...
    items = items.filter((r) => idSet.has(r.chuc_vu_id));
  }
  return items
    .map(fromSheetRow)
    .sort((a, b) => a.chuc_vu_id.localeCompare(b.chuc_vu_id, 'vi') || a.module_key.localeCompare(b.module_key));
}
...
export async function replacePhanQuyenForModule(params: {
  moduleKey: string;
  rows: Array<{ chuc_vu_id: string; quyen: string }>;
  ...
  if (chucVuIds.length > 0) {
    ...
    const toDelete = existing.filter(
      (r) => r.module_key === moduleKey && chucVuIds.includes(r.chuc_vu_id),
    );
 ...
 */
export async function assertChucVuIdsExist(_ids: string[]): Promise<string[]> {
  return [];
}

export async function findQuyenCsvByChucVuAndModule(chucVuId: string, moduleKey: string): Promise<string> {
  const { rows } = await readTable(TAB);
  const row = rows.find((r) => r.chuc_vu_id === chucVuId && r.module_key === moduleKey);
  return row?.quyen ?? '';
}
