...
describe('assertPhongBanPermission', () => {
  beforeEach(() => {
  ...
  it('allows super user (cap_bac === 1)', async () => {
    ...
    expect(await assertPhongBanPermission(c, 'xoa')).toBeNull();
  });
  ...
  it('allows when matrix has them', async () => {
    ...
    expect(await assertPhongBanPermission(c, 'them')).toBeNull();
  });
  ...
  it('allows sua for creator without sua token', async () => {
    ...
    const c = mockContext({ employee_id: '2', tai_khoan: 'nv', cap_bac: 4 });
    expect(
      await assertPhongBanPermission(c, 'sua', { recordNguoiTao: '2' }),
    ).toBeNull();
  });
});
