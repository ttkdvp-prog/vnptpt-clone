...
const MODULE_DANH_SACH_TAI_LIEU = 'danh_sach_tai_lieu';

export async function assertDanhSachTaiLieuPermission(
  c: Context,
  action: ModuleAction,
  opts?: { recordNguoiTao?: string | null },
): Promise<Response | null> {
  return assertModulePermission(c, MODULE_DANH_SACH_TAI_LIEU, action, {
    recordNguoiTao: opts?.recordNguoiTao,
    allowOwnRow: true,
  });
}

export interface DocumentListViewer {
  bypassAcl: boolean;
  employeeId: number | null;
  chucVuId: number | null;
}

/** Super / module admin bỏ qua ACL mảng; còn lại lọc theo chức vụ / NV / người tạo. */
export async function resolveDocumentListViewer(c: Context): Promise<DocumentListViewer> {
  ...
  const quyen =
    chucVuId != null
      ? await findQuyenCsvByChucVuAndModule(chucVuId, MODULE_DANH_SACH_TAI_LIEU)
      : '';
