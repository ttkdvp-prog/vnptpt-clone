-- Phiếu hành chính (workflow duyệt QL → HCNS)
...
CREATE TABLE IF NOT EXISTS "cong_luong_phieu_hanh_chinh" (
    "id" SERIAL NOT NULL,
    "id_loai_phieu" INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS "cong_luong_phieu_hanh_chinh_id_loai_phieu_idx"
  ON "cong_luong_phieu_hanh_chinh"("id_loai_phieu");
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cong_luong_phieu_hanh_chinh_id_loai_phieu_fkey'
  ) THEN
    ALTER TABLE "cong_luong_phieu_hanh_chinh"
      ADD CONSTRAINT "cong_luong_phieu_hanh_chinh_id_loai_phieu_fkey"
      FOREIGN KEY ("id_loai_phieu") REFERENCES "cong_luong_nhom_phieu_hanh_chinh"("id")
...
-- Seed: chỉ khi bảng trống; cover đủ 4 trạng thái + buổi sang/chieu/dem
INSERT INTO "cong_luong_phieu_hanh_chinh"
  ("id_loai_phieu", "id_nhan_vien", "tu_ngay", "buoi_bat_dau", "den_ngay", "buoi_ket_thuc",
...
FROM (
  VALUES
    ...
    ('NP', 1, '2026-06-20', 'chieu', '2026-06-22', 'sang', NULL, NULL, 'Nghỉ phép năm — trùng lịch', 'tu_choi', true),
...
JOIN "cong_luong_nhom_phieu_hanh_chinh" lp ON lp."ma_phieu" = s.ma_phieu
JOIN LATERAL (
